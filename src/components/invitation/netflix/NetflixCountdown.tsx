...
interface NetflixCountdownProps {
...
export default function NetflixCountdown({ targetDateISO }: NetflixCountdownProps) {
  ...
  return (
    <section className="py-4 md:py-6 w-full px-4 sm:px-8 md:px-12 lg:px-16 text-white select-none space-y-4">
      ...
      {/* Main Container Card Widescreen Full-Width */}
      <motion.div
        initial={{ opacity: 0, y: 15 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.5 }}
        className="w-full bg-gradient-to-r from-[#260507] via-[#1A1A1A] to-[#1F1F1F] border border-zinc-800 rounded-2xl p-6 md:p-8 shadow-2xl flex flex-col lg:flex-row lg:items-center justify-between gap-6"
      >
        ...
        {/* Right Side: 3 Calendar Buttons */}
        <div className="flex flex-col items-start lg:items-end space-y-2">
          <div className="flex flex-wrap items-center gap-2.5">
            <button
              onClick={handleGoogleCalendar}
              className="flex items-center gap-2 bg-transparent hover:bg-white/10 border border-zinc-600 text-xs font-semibold px-4 py-2 rounded-lg text-white transition-all hover:border-white"
            >
